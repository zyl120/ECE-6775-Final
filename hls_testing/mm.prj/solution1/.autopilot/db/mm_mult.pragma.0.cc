...
void tiling_systolic_helper (
    ...
    int ii,
    int jj
) {_ssdm_SpecArrayDimSize(A, 100);_ssdm_SpecArrayDimSize(B, 100);_ssdm_SpecArrayDimSize(out, 100);
    for(int k = 0; k < 100; k++){
#pragma HLS PIPELINE II=1
 for(int i = 0; i < 4; i++) {
            for(int j = 0; j < 4; j++) {
                DTYPE last = (k == 0) ? 0 : out[i+ii][j+jj];
                DTYPE a_val = (i+ii < 100 && k < 100) ? A[i+ii][k] : 0;
                DTYPE b_val = (k < 100 && j+jj < 100) ? B[k][j+jj] : 0;
 ...
 systolic2:
        for (int i = 0; i < 100; i++) {
            ...
            for (int j = 0; j < 100; j++) {
                ...
                short a_val = (i < 100 && k < 100) ? a[i][k] : 0;
                short b_val = (k < 100 && j < 100) ? b[k][j] : 0;
