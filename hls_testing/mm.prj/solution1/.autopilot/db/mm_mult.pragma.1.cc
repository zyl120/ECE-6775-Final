...
void tiling_systolic_helper (
    ...
    int ii,
    int jj
) {_ssdm_SpecArrayDimSize(A, 10);_ssdm_SpecArrayDimSize(B, 10);_ssdm_SpecArrayDimSize(out, 10);
    for(int k = 0; k < 10; k++){
_ssdm_op_SpecPipeline(1, 1, 1, 0, "");
 for(int i = 0; i < 2; i++) {
            for(int j = 0; j < 2; j++) {
                DTYPE last = (k == 0) ? 0 : out[i+ii][j+jj];
                DTYPE a_val = (i+ii < 10 && k < 10) ? A[i+ii][k] : 0;
                DTYPE b_val = (k < 10 && j+jj < 10) ? B[k][j+jj] : 0;
 ...
 systolic2:
        for (int i = 0; i < 10; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                ...
                DTYPE a_val = (i < 10 && k < 10) ? a[i][k] : 0;
                DTYPE b_val = (k < 10 && j < 10) ? b[k][j] : 0;
                DTYPE result = last + a_val * b_val;
                out[i][j] = result;
            }
        }
